...
ACTIVATION_WEIGHTS = {
    ...
    "roi_normalised": 0.30,
    "revenue_per_click": 0.20,
    "satisfaction_normalised": 0.15,
}

SUBSCRIPTION_TIER_WEIGHTS = {"Premium": 3, "Standard": 2, "Basic": 1, "Unknown": 1}
...
def add_discount_tier(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["discount_tier"] = pd.cut(df["Discount_Level"], bins=bins, labels=labels, include_lowest=True).astype(str)
    return df
...
def add_activation_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    components = {
        ...
        "satisfaction_normalised": df["Customer_Satisfaction_Post_Refund"].astype(float).values.reshape(-1, 1),
    }
    ...
    df["activation_label"] = pd.cut(
        df["activation_score"], bins=[-np.inf, p33, p67, np.inf], labels=["Low", "Medium", "High"]
    ).astype(str)
    print(f"[features] Activation score range: {df['activation_score'].min():.2f} – {df['activation_score'].max():.2f}")
    return df
...
def add_vanity_flag(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"[features] Vanity traffic campaigns detected: {df['vanity_traffic_flag'].sum():,}")
    return df
