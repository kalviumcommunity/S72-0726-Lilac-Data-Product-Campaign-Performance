"""
conversion_predictor.py - Predicts conversion rate using XGBoost Regressor
"""
import json
...
def run(df: pd.DataFrame = None) -> dict:
    ...
    print("\n[conversion_predictor] --- Training Conversion Predictor ---")
    ...
    joblib.dump(model, MODELS_DIR / "conversion_predictor.joblib")
    
    # Feature importances
    importances = dict(zip(features, [float(v) for v in model.feature_importances_]))
    with open(PROCESSED_DIR / "ml_conversion_importance.json", "w") as f:
        json.dump(importances, f, indent=2)
        
    # Test predictions
    # Get original Campaign_ID if available, else just indices
    ...
    with open(PROCESSED_DIR / "ml_conversion_predictions.json", "w") as f:
        json.dump(predictions, f, indent=2)
        
    with open(PROCESSED_DIR / "ml_conversion_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
        
    print(f"[conversion_predictor] Metrics: {metrics}")
    return metrics
